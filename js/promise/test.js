/**
 1.Promise.resolve()方法可以将现有对象转为Promise对象。
 2.Promise.reject()方法可以将现有对象转为Promise对象，并将其状态设置为rejected。
 3.Promise.all()方法可以将多个Promise对象包装成一个新的Promise对象，该对象只有当所有传入的Promise对象都被resolve或reject后才会被resolve或reject。
 4.Promise.allSettled()方法可以将多个Promise对象包装成一个新的Promise对象，该对象会包含所有传入的Promise对象的状态。
 5.Promise.race()方法可以将多个Promise对象包装成一个新的Promise对象，该对象会在第一个Promise对象被resolve或reject后，将其状态传递给新的Promise对象。
 6.Promise.any()方法可以将多个Promise对象包装成一个新的Promise对象，该对象只有当传入的Promise对象中有一个被resolve或reject后才会被resolve或reject。
 7.then
 */
//手写promise的实现
class myPromise {
